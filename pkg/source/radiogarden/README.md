...
## Purpose
Provides live radio streams from cities worldwide via the Radio Garden API (radio.garden). Each source instance is configured with one or more places (cities) and fetches all radio stations available for those locations. The source type is `radiogarden`.

## Responsibilities
- Fetch channels for all configured places from the Radio Garden API
...
- Fetch all places -- the user configures places in the source config, the source fetches channels for those places
