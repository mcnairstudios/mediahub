...
func TestKeyframeTracker_Reset(t *testing.T) {
	tracker := NewKeyframeTracker(false)
	idr := []byte{0x00, 0x00, 0x00, 0x01, 0x65, 0xBB}
	nonIDR := []byte{0x00, 0x00, 0x00, 0x01, 0x41, 0xAA}

	if tracker.ShouldDrop(idr, "h264") {
		t.Error("expected IDR to not be dropped")
	}
	tracker.Reset()
	if !tracker.ShouldDrop(nonIDR, "h264") {
		t.Error("expected non-IDR to be dropped after reset")
	}
}

func TestIsKeyframe_H265_AllIDRTypes(t *testing.T) {
	for nalType := byte(16); nalType <= 21; nalType++ {
		encoded := nalType << 1
		data := []byte{0x00, 0x00, 0x00, 0x01, encoded, 0x01, 0xAA}
		if !IsKeyframe(data, "hevc") {
			t.Errorf("expected H265 NAL type %d to be detected as keyframe", nalType)
		}
	}
}

func TestIsKeyframe_H265_H265Alias(t *testing.T) {
	data := []byte{0x00, 0x00, 0x00, 0x01, 0x26, 0x01, 0xAA}
	if !IsKeyframe(data, "h265") {
		t.Error("expected h265 alias to work same as hevc")
	}
}

func TestSplitNALUnits_FourByteStartCode(t *testing.T) {
	data := []byte{0x00, 0x00, 0x00, 0x01, 0x65, 0xAA, 0x00, 0x00, 0x00, 0x01, 0x41, 0xBB}
	nalus := splitNALUnits(data)
	if len(nalus) != 2 {
		t.Fatalf("expected 2 NAL units, got %d", len(nalus))
	}
	if nalus[0][0] != 0x65 {
		t.Error("expected first NAL to be IDR")
	}
	if nalus[1][0] != 0x41 {
		t.Error("expected second NAL to be non-IDR")
	}
}

func TestSplitNALUnits_ThreeByteStartCode(t *testing.T) {
	data := []byte{0x00, 0x00, 0x01, 0x65, 0xAA, 0x00, 0x00, 0x01, 0x41, 0xBB}
	nalus := splitNALUnits(data)
	if len(nalus) != 2 {
		t.Fatalf("expected 2 NAL units, got %d", len(nalus))
	}
...
func TestSplitNALUnits_Empty(t *testing.T) {
	nalus := splitNALUnits(nil)
	if len(nalus) != 0 {
		t.Errorf("expected 0 NAL units from nil, got %d", len(nalus))
	}
	nalus = splitNALUnits([]byte{})
	if len(nalus) != 0 {
		t.Errorf("expected 0 NAL units from empty, got %d", len(nalus))
	}
}
...
func TestSplitNALUnits_MixedStartCodes(t *testing.T) {
	...
	if nalus[0][0] != 0x67 {
		t.Error("expected first NAL to be SPS (0x67)")
	}
	if nalus[1][0] != 0x68 {
		t.Error("expected second NAL to be PPS (0x68)")
	}
}
