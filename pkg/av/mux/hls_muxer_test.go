...
func TestNewHLSMuxer_EmptyDir(t *testing.T) {
	_, err := NewHLSMuxer(HLSMuxOpts{})
	if err == nil {
		t.Fatal("expected error for empty OutputDir")
	}
}

func TestHLSMuxer_CloseIdempotent(t *testing.T) {
	dir := t.TempDir()
...
func TestHLSMuxer_WriteAfterClose(t *testing.T) {
	dir := t.TempDir()
...
func TestHLSMuxer_DefaultSegmentDuration(t *testing.T) {
	dir := t.TempDir()
	...
	if m.opts.SegmentDurationSec != 6 {
		t.Errorf("default segment duration = %d, want 6", m.opts.SegmentDurationSec)
	}
}
