...
func finishDemuxerSetup(fc *astiav.FormatContext, url string, opts DemuxOpts) (*Demuxer, error) {
	var pr *media.ProbeResult
	if opts.CachedStreamInfo != nil {
		pr = opts.CachedStreamInfo
	} else {
		...
		pr = probe.ExtractProbeResult(fc)
	}
	...
	if len(dm.streams) > 0 {
		...
		dm.setIndicesFromCachedInfo(pr, opts)
	}
...
func (d *Demuxer) setIndicesFromCachedInfo(pr *media.ProbeResult, opts DemuxOpts) {
	if pr.Video != nil {
		d.videoIdx = pr.Video.Index
	}
	if len(pr.SubTracks) > 0 {
		d.subIdx = pr.SubTracks[0].Index
	}

	var audioCandidates []audioCandidate
	for _, at := range pr.AudioTracks {
...
func (d *Demuxer) readPacketOnce() (*av.Packet, error) {
	for {
		select {
		case req := <-d.seekCh:
